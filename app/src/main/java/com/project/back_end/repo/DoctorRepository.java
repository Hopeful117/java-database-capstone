...
public interface DoctorRepository extends JpaRepository<Doctor, Long> {
    ...
    @Query("SELECT d FROM Doctor d WHERE d.last_name LIKE CONCAT('%', :name, '%')")
    List<Doctor> findByNameLike(@Param("name") String name);

    @Query("SELECT d FROM Doctor d WHERE LOWER(d.last_name) LIKE LOWER(CONCAT('%', :name, '%')) AND LOWER(d.specialty) = LOWER(:specialty)")
